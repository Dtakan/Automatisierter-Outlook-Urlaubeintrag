...
class MainWindow:
    ...
    def create_widgets(self):
        ...
        self.browse_button = tk.Button(self.master, text='Dateipfad Durchsuchen', command=self.browse)
        self.instructions = tk.Button(self.master, text='Anleitung', command=self.show_instructions)
        self.plug = tk.Button(self.master, text='About', command=self.show_plug)
        ...
        self.name_label.grid(row=1, column=0, padx=15, pady=5, sticky='W')
        self.name_entry.grid(row=1, column=1, padx=5, pady=5)
        self.output_label.grid(row=2, column=0, padx=15, pady=5, sticky='W')
        self.output_entry.grid(row=2, column=1, padx=5, pady=5)
        self.browse_button.grid(row=2, column=2, padx=5, pady=5)
        self.instructions.grid(row=0, column=1, padx=5, pady=5)
        self.plug.grid(row=0, column=3, padx=5, pady=5)
        self.information.grid(row=0, column=0, padx=5, pady=5)
        self.run_script.grid(row=3, column=3, padx=5, pady=5)
    
    def show_instructions(self):
        messagebox.showinfo('Instructions', 'Sie müssen Outlook und die Seite mit Ihren Urlaubsdaten (zB Zeus) geöffnet haben:\n\n'
                            '1. Geben Sie Ihren Nachnamen an.\n'
                            '2. Wählen Sie ein Dateipfad aus, in dem die .txt Datei gespeichert werden soll\n'
                            ...
                            "Das war's auch schon - Ihre Urlaubsdaten sind in Ihrem Outlook Kalender mit dem Betreff 'Urlaub IhrNachname' eingetragen :-)")
        
    def show_information(self):
        messagebox.showinfo("Allgemeine Information", "Dieses Skript ist ein Bedienfeld-basiertes Tool, das es dem Benutzer ermöglicht, seine Urlaubsdaten in seinen Outlook-Kalender einzutragen. Der Benutzer muss lediglich seinen Namen eingeben und einen Speicherort auswählen, wo das Skript die erstellte .txt-Datei mit den Urlaubsdaten speichern soll (diese können später gelöscht werden, wenn gewünscht). Durch Klicken auf den 'Skript starten' Button werden die Urlaubsdaten automatisch in den Outlook-Kalender eingetragen.")
    ...
    def run_script(self):
        ...
        for start_date, end_date in valid_dates:
            ...
            appointment.Subject = 'Urlaub ' + self.name_entry.get()
